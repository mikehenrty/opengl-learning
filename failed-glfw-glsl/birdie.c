...
static int make_resources(void) {
  ...
  if (g_resources.vertex_shader == 0) {
    fprintf(stderr, "Failed to load vertext shader\n");
    return 0;
  }
  ...
  if (g_resources.program == 0) {
    fprintf(stderr, "Failed to load opengl program\n");
    return 0;
  }
...
static void render(void)
{
  ...
  GLuint vao;
  glGenVertexArrays (1, &g_resources.vertex_buffer);
...
int main(int argc, char** argv) {
  ...
  GLFWwindow* window = glfwCreateWindow(640, 480, "Birdie", NULL, NULL);
  if (!window) {
    glfwTerminate();
    return EXIT_FAILURE;
  }
  ...
  int iterations = 0;
  while (!glfwWindowShouldClose(window) && iterations < 100) {
    update_fade_factor();
    render();
    glfwSwapBuffers(window);
    glfwPollEvents();
  ...
  print_programme_info_log(g_resources.program);
